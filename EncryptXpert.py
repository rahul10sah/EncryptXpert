...
ERRORS = {
	...
	'[F_UC-0]':"Please check back later or contact with Rahul Sah. Probably there is an update on the way!",
...
'''
Rahul Sah (Github: www.github.com/rahul10sah ), Nepal
'''
...
class UI(QMainWindow):
	...
	def __init__(self):
		...
		ui_path = os.path.join(script_dir, "EX_GUI.ui")

		# Load the UI using the absolute path
	...
	def UpdateChecker(self):
		...
		try:
			try:
				url_response = requests.get("https://raw.githubusercontent.com/rahul10sah/EncryptXpert/main/EncryptXpert.py").text.split('\n')
	...
	def LabelSwitcher(self, label_name):
		if label_name == 'enc_key_label':
			if self.enc_key_label.text() == 'Key (B):':
				...
				self.enc_key_input.setPlaceholderText('Type your key in Bytes format (example: Rahul123)')
			...
			if self.dec_key_label.text() == 'Key (B):':
				...
				self.dec_key_input.setPlaceholderText('Type your key in Bytes format (example: Rahul123)')
...
# =========================
# PKI LOGIN (Single-file)
# =========================

CURRENT_USER_NAME = None
CURRENT_USER_P12 = None
...
def _ca_paths():
    base = _base_dir()
    ca_dir = os.path.join(base, "data", "ca")
    ca_key = os.path.join(ca_dir, "ca_private_key.pem")
    ca_cert = os.path.join(ca_dir, "ca_certificate.pem")
    return ca_dir, ca_key, ca_cert

def ensure_local_ca_exists():
    """
    Creates a local Root CA if it doesn't exist.
    This prevents the 'CA certificate not found' popup.
    """
    ca_dir, ca_key_path, ca_cert_path = _ca_paths()
    ...
    subject = issuer = x509.Name([
        x509.NameAttribute(x509.oid.NameOID.COUNTRY_NAME, "NP"),
        x509.NameAttribute(x509.oid.NameOID.ORGANIZATION_NAME, "EncryptXpert"),
        x509.NameAttribute(x509.oid.NameOID.COMMON_NAME, "EncryptXpert Local Root CA"),
    ])
...
def load_identity_from_p12(p12_path: str, password: str):
    """
    Loads (private_key, certificate, additional_certs) from PKCS#12.
    """
    ...
    if key is None or cert is None:
        raise ValueError("Invalid .p12/.pfx or wrong password.")
    return key, cert, addl

def validate_cert_against_ca(user_cert: x509.Certificate, ca_cert: x509.Certificate) -> tuple[bool, str]:
    """
    Minimal trust validation:
      - validity time
      - issuer matches CA subject
      - CA signature verifies user cert
    """
    now = datetime.now(timezone.utc)

    # Compatibility for different cryptography versions
    try:
        not_before = user_cert.not_valid_before_utc
        not_after = user_cert.not_valid_after_utc
    except AttributeError:
        not_before = user_cert.not_valid_before.replace(tzinfo=timezone.utc)
        not_after = user_cert.not_valid_after.replace(tzinfo=timezone.utc)
    ...
    if user_cert.issuer != ca_cert.subject:
        return False, "Certificate issuer mismatch (not issued by this CA)."
    ...
    try:
        ...
        return False, "Certificate signature invalid (not signed by CA)."
    except Exception as e:
        return False, f"Certificate validation error: {e}"

    return True, "Certificate trusted under CA."

def challenge_response(private_key, user_cert: x509.Certificate) -> tuple[bool, str]:
    """
    Proves user owns the private key:
      - Tool generates random nonce
      - User signs nonce with private key
      - Tool verifies using public key from certificate
    """
    nonce = os.urandom(32)
    ...
    try:
        if isinstance(private_key, rsa.RSAPrivateKey):
            ...
            return False, "Only RSA keys supported in this build."
    except Exception as e:
        return False, f"Signing failed: {e}"

    # Verify using cert public key
    ...
    try:
        ...
        return False, "Challenge-response failed (invalid signature)."
    except Exception as e:
        return False, f"Verification error: {e}"

    return True, "Challenge-response passed."

def _get_cn(cert: x509.Certificate):
...
def authenticate():
    """
    Final PKI Login for Coursework:
      1) Ensure CA exists (creates if missing)
      2) User selects identity.p12 / identity.pfx
      3) User enters password
      4) Validate certificate under CA
      5) Challenge-response signature
    """
    # Ensure CA exists (creates automatically if missing)
    _, ca_cert_path = ensure_local_ca_exists()
    ca_cert = load_ca_cert(ca_cert_path)

    # Select p12/pfx
    p12_file = QFileDialog.getOpenFileName(
        None,
        "Select Your Identity (.p12/.pfx)",
        _base_dir(),
        "PKCS#12 Files (*.p12 *.pfx);;All Files (*.*)"
    )[0]
    if not p12_file:
        QMessageBox.critical(None, "Login Cancelled", "No identity file selected.")
        return False

    # Enter password
    p12_pass, ok = QInputDialog.getText(
        None,
        "PKCS#12 Password",
        "Enter your identity password:",
        QLineEdit.Password
    )
    if not ok or not p12_pass:
        QMessageBox.critical(None, "Login Failed", "No password entered.")
        return False

    # Load identity
    try:
        priv, user_cert, _ = load_identity_from_p12(p12_file, p12_pass)
    except Exception as e:
        QMessageBox.critical(None, "Login Failed", f"Cannot load identity: {e}")
        return False

    # Validate certificate trust
    ok_cert, msg_cert = validate_cert_against_ca(user_cert, ca_cert)
    if not ok_cert:
        QMessageBox.critical(None, "Access Denied", msg_cert)
        return False

    # Challenge-response
    ok_cr, msg_cr = challenge_response(priv, user_cert)
    if not ok_cr:
        QMessageBox.critical(None, "Access Denied", msg_cr)
        return False

    # Success
    global CURRENT_USER_NAME, CURRENT_USER_P12
    CURRENT_USER_NAME = _get_cn(user_cert)
    CURRENT_USER_P12 = p12_file

    QMessageBox.information(
        None,
        "Login Success",
        f"Welcome {CURRENT_USER_NAME}!\n\n"
        f"Certificate validated under CA.\n"
        f"Challenge-response authentication passed."
    )
    return True
...
if __name__ == "__main__":
    ...
    # PKI AUTHENTICATION FIRST
    if not authenticate():
        sys.exit()

    UIWindow = UI()
    UIWindow.SetDefaultOptions()

    if UIWindow.option_Check_Update_on_program_startup.isChecked():
